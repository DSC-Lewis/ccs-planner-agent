...
def test_automatic_flow_distributes_budget_respecting_constraints(owner_id):
    session = agent.create_session(SessionMode.AUTOMATIC, owner_id=owner_id)

    session = _adv(session, owner_id, survey_id="tw_2025", client_id="internal_pitch")
    session = _adv(session, owner_id, project_name="auto test",
                   start_date="2026-02-16", weeks=4)
    session = _adv(session, owner_id, target_ids=["all_adults"])
    session = _adv(session, owner_id, planning_type="Comm")
    session = _adv(session, owner_id, comms={
    ...
    session = _adv(session, owner_id, channel_ids=[
        "tv_advertising", "youtube_video_ads", "meta_video_ads",
    ])
    assert session.step == StepKey.CRITERION

    session = _adv(session, owner_id, criterion_id="net_reach", strategy_id="global_plan")
